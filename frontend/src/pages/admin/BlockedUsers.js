...
const BlockedUsers = () => {
  ...
  const fetchLockedUsers = async () => {
    try {
      setLoading(true);
      const res = await axios.get("https://api.getfitindia.in/api/auth/locked-users", {
  ...
  const handleUnlock = async (userId) => {
    try {
      const res = await axios.put(`https://api.getfitindia.in/api/auth/unlock-user/${userId}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
